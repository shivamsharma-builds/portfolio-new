...
function normalizeSettings(value) {
  ...
  settings.aboutParagraphs = arrayOr(settings.aboutParagraphs, defaultSite.settings.aboutParagraphs).filter(Boolean).map(String)
  return settings
}

function normalizeItem(section, item, index) {
  if (section === 'additionalSkills') return typeof item === 'string' ? item : String(item?.name ?? item?.value ?? '')
  const value = objectOr(item, {})
  const base = defaultSite[section]?.[0] || {}
  const normalized = { ...base, ...value, id: value.id ?? `local-${section}-${index}` }
  if (section === 'education') normalized.courses = arrayOr(normalized.courses, []).filter(Boolean).map(String)
  if (section === 'experience') normalized.achievements = arrayOr(normalized.achievements, []).filter(Boolean).map(String)
  if (section === 'projects') normalized.tags = arrayOr(normalized.tags, []).filter(Boolean).map(String)
  if (section === 'skills') normalized.level = Math.max(0, Math.min(100, Number(normalized.level) || 0))
...
function mergeSiteData(remote) {
  ...
  const result = { ...defaultSite, ...remote, settings: normalizeSettings(remote.settings) }
  for (const section of ['socials','navLinks','stats','skills','additionalSkills','education','experience','projects','certificates']) {
    result[section] = arrayOr(remote[section], defaultSite[section]).map((item, index) => normalizeItem(section, item, index))
  }
  return result
}
...
export function SiteDataProvider({ children }) {
  ...
  const value = useMemo(() => ({ data, loading, error, refresh, ...data }), [data, loading, error, refresh])
