...
function PortfolioApp(){
  const { data, loading: dataLoading, error: dataError, refresh } = useSiteData()
  ...
  if(loading)return <div className="flex min-h-screen items-center justify-center bg-[#0a0a0f] px-6"><div className="w-full max-w-md text-center"><div className="mx-auto mb-4 h-12 w-12 animate-spin rounded-full border-4 border-orange-500 border-t-transparent"/><p className="font-bold text-white">Loading latest portfolio content...</p><p className="mt-2 text-sm text-gray-400">Fetching the current data from the database.</p></div></div>
  if(dataError)return <div className="flex min-h-screen items-center justify-center bg-[#0a0a0f] px-6 text-white"><div className="max-w-lg rounded-2xl border border-orange-500/20 bg-white/5 p-6 text-center"><h1 className="text-xl font-bold">Unable to load portfolio</h1><p className="mt-2 break-words text-sm text-gray-400">{dataError}</p><button type="button" onClick={refresh} className="mt-5 rounded-xl bg-orange-500 px-5 py-3 font-semibold text-black">Try again</button></div></div>
